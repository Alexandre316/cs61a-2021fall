...
def flatten(s):
    ...
    if len(s) == 1:
        ...
        result = flatten(s[0]) + flatten(s[1:])
    else:
        result = [s[0]] + flatten(s[1:])
    return result
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    return get_name(city_a) if sqrt((get_lat(city_a)-lat)**2+(get_lon(city_a)-lon)**2) <\
    sqrt((lat-get_lat(city_b))**2+(lon-get_lon(city_b))**2) else get_name(city_b)

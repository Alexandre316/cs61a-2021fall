...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def in_condition(words):
        for word in lower(words).split():
            if remove_punctuation(word) in topic:
                return True
        return False
    return in_condition
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0 and len(reference_words) == 0:
        return float(100)
    elif len(typed_words)*len(reference_words) == 0:
        return float(0)
    else:
        for ix, word in enumerate(typed_words):
            try:
                if word == reference_words[ix]:
                    accuracy_pct += 100/len(typed_words)
            except IndexError:
                continue
    return accuracy_pct
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    Arguments:
        ...
        limit: if the lowest difference between TYPED_WORD and any of the VALID_WORDS is greater than LIMIT, 
        then TYPED_WORD is returned instead.
    ...
    if typed_word in valid_words:
        return typed_word
    words_diff = [diff_function(typed_word, word, limit) for word in valid_words]
    lowest_idx, lowest = 0, words_diff[0]
    for ix, diff in enumerate(words_diff):
        if diff < lowest:
            lowest_idx = ix
            lowest = diff
    if lowest > limit:
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if len(start) == 0 or len(goal) == 0 or limit < 0:
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    progress:float = 0.0
    for ix, word in enumerate(sofar):
        if word == prompt[ix]:
            progress += 1
        else:
            break
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    times = [[times_used[i+1]-times_used[i] for i in range(len(times_used)-1)] for times_used in times_per_player]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    record_for_players = [[] for _ in player_indices]
    for idx in word_indices:
        player, record_until = 0, get_times(match)[0][idx]
        for p in player_indices:
            if get_times(match)[p][idx] < record_until:
                player, record_until = p, get_times(match)[p][idx]
        record_for_players[player].append(get_words(match)[idx])
    return record_for_players
    # END PROBLEM 10

...
def roll_dice(num_rolls, dice=six_sided):
    ...
    rolled_dice:int = 0
    outcomes:int = 0
    pig_outcomes:bool = False
    while rolled_dice<num_rolls:
        points = dice()
        outcomes += points
        if points == 1:
            pig_outcomes = True
...
def picky_piggy(score):
    ...
    if score == 0:
        ...
        order_num = (score-1)%6+1
        return PIGGY_DIGIT//(10**(order_num-1)) - PIGGY_DIGIT//(10**order_num)*10
    # END PROBLEM 2
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def select_player(score0, score1):
        tmp_high = running_high # cannot use nonlocal
        if eval(f"score{who}")-last_score>tmp_high:
            tmp_high = eval(f"score{who}")-last_score
            print("{0} point(s)! That's a record gain for Player {1}!".format(tmp_high, who))
        return announce_highest(who, eval(f"score{who}"), tmp_high)
...
def make_averaged(original_function, trials_count=1000):
    ...
    def return_avg(*args):
        ...
        for _ in range(trials_count):
            result += float(original_function(*args))
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_dice, max_avg, tmp_avg = 0, 0, 0
    for i in range(1, 11):
        tmp_avg += make_averaged(roll_dice, trials_count)(i, dice)
        if tmp_avg > max_avg:
            max_avg, max_dice = tmp_avg, i
        tmp_avg = 0
    return max_dice
    # END PROBLEM 9
...
def picky_piggy_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    if picky_piggy(opponent_score)>=cutoff and score<GOAL_SCORE:
        return 0
    return num_rolls
    # END PROBLEM 10
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if score+picky_piggy(opponent_score) == opponent_score or picky_piggy(opponent_score) >= cutoff:
        return 0
    return num_rolls
    # END PROBLEM 11

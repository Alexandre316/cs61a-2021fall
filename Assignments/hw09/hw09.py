...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r'(cs|CS|cS|Cs) ?\d+[abcABC]?', post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'\b(?:(?:[0-1]?[0-9])|(?:[2][0-4])):[0-5][0-9](?:AM|PM|am|pm)?\b', text)
...
def most_common_code(text):
    ...
    return max(codes, key = lambda x:codes.count(x)) # 传入命名参数key，其为一个函数，用来指定取最大值的方法

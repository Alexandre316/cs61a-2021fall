...
def pingpong(n):
    ...
    def diff(n):
        if n < 8:
            return 1
        return diff(n-1) * (-1 if num_eights(n-1) or (n-1) % 8 == 0 else 1)
...
def missing_digits(n):
    ...
    "*** YOUR CODE HERE ***"
    # next_digit:int = n%10
    # num_of_missing:int = 0
    # while n >= 10:
    #     tmp_digit = n%10
    #     next_digit = n//10%10
    #     num_of_missing += max((tmp_digit-next_digit-1),0)
    #     n = n//10
...
def count_coins(change):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_changes(num_change, n):
        if num_change == 0:
            return 1
        elif num_change < 0:
            return 0
            ...
            origin = count_changes(num_change, ascending_coin(n))
            remainder = count_changes(num_change - n, n)
            return origin + remainder
    return count_changes(change, 1)

...
class VendingMachine:
    ...
    def __init__(self, product_name, price):
        ...
        self.price = price
        self.stock = 0
        self.balance_until = 0
    
    def vend(self):
        if self.balance_until > self.price:
            change = self.balance_until - self.price
            self.balance_until = 0
            self.stock -= 1
            return f"Here is your {self.product_name} and ${change} change."
        elif self.balance_until == self.price:
            self.balance_until -= self.price
            self.stock -= 1
            return  f"Here is your {self.product_name}."
        elif self.stock == 0:
            return 'Nothing left to vend. Please restock.'
        else:
            return f"You must add ${self.price - self.balance_until} more funds."

    def add_funds(self, funds):
        if self.stock == 0:
            return f"Nothing left to vend. Please restock. Here is your ${funds}."
        else:
            self.balance_until += funds
            return f'Current balance: ${self.balance_until}'
...
def store_digits(n):
    ...
    def rest(n):
        ...
        if n >= 10:
            rest(n//10)
        if isinstance(main_link, Link):
            pointer.rest = Link(n%10)    
...
def two_list(vals, amounts):
    ...
    "*** YOUR CODE HERE ***"
    main_link = None
    for ix, number in enumerate(amounts[::-1]):
        for _ in range(number):
            if main_link == None:
                main_link = Link(vals[::-1][ix])
            else:
                main_link = Link(vals[::-1][ix], main_link)
    return main_link
